...
> R:24 falta multiplicar por cantidad

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Esa línea debería multiplicar el precio por la cantidad de ese producto
...
> R:Con el bug todavía presente, el test ni siquiera llega a evaluar el ASSERT_IGUAL
...
> R:En src/carrito.c, la condición es if (c->cantidad <= MAX_ITEMS). El operador <= es incorrecto; debería ser <. Con <=, cuando cantidad vale 4 (carrito ya lleno), la condición 4 <= 4 sigue siendo verdadera y permite escribir fuera de rango
...
> R:Sí, las líneas 29 y 30 de `carrito.c`, correspondientes a la función completa `carrito_descuento`, aparecen marcadas con `#####`. No se ejecutaron porque `make cobertura` mide la cobertura usando solo `test_unitarios.c` (junto con `carrito.c`), y ninguno de los tests unitarios llama a `carrito_descuento` — esa función solo se usa en `programa.c` y en el test de integración de la Parte 8, que no forma parte de esta medición de cobertura.
...
> R:Un test unitario verifica una función de manera aislada, con entradas controladas y mínimas, para chequear que esa pieza puntual del código funciona como se espera. Un test de integración, en cambio, verifica que varias funciones trabajen bien combinadas entre sí, simulando un flujo más parecido al uso real. El test **unitario**  fue el que detectó primero el bug de `carrito_total`, antes de llegar siquiera al test de integración.
...
> R:En C, los arrays no tienen chequeo de límites automático: el lenguaje le permite al programa escribir en cualquier posición de memoria, incluso fuera del array declarado, y simplemente sobreescribe lo que haya en esa dirección de memoria , lo que puede corromper el estado del programa o crashearlo de forma impredecible.
...
> R:Escribir tests obliga a definir de antemano cuál es el resultado correcto esperado, y después comparar ese valor con lo que realmente devuelve el código — de esa forma el bug se detecta de manera objetiva y automática, sin depender de que una persona lo note "a simple vista" leyendo el código. Mirar el código directamente es más propenso a errores humanos.
...
> R:Porque cuando la cantidad es 1, multiplicar por la cantidad no cambia el resultado (`precio * 1 = precio`), entonces el bug de `carrito_total` (que sumaba solo `precio` sin multiplicar por `cantidad`) quedaba oculto: el resultado incorrecto coincidía por casualidad con el resultado correcto. Recién con cantidad = 2 la diferencia entre "sumar solo el precio" y "sumar precio × cantidad" se hace evidente (350 vs 700), y ahí el test detecta el bug.
```
